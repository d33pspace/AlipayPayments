...
namespace Com.Alipay
{
    ...
    public class Notify
    {
        ...
        public async Task<bool> Verify(SortedDictionary<string, string> inputPara, string notify_id, string sign)
        {
            //获取返回时的签名验证结果
            bool isSign = GetSignVeryfy(inputPara, sign);
            //获取是否是支付宝服务器发来的请求的验证结果
            string responseTxt = "false";
            if (notify_id != null && notify_id != "") { responseTxt = await GetResponseTxt(notify_id); }
        ...
        /// <param name="notify_id">通知验证ID</param>
        /// <returns>验证结果</returns>
        private async Task<string> GetResponseTxt(string notify_id)
        {
            string veryfy_url = Https_veryfy_url + "partner=" + _partner + "&notify_id=" + notify_id;

            //获取远程服务器ATN结果，验证是否是支付宝服务器发来的请求
            string responseTxt = await Get_HttpAsync(veryfy_url, 120000);
        ...
        private async System.Threading.Tasks.Task<string> Get_HttpAsync(string strUrl, int timeout)
        {
            string strResult;
            try
            {
                var request = HttpWebRequest.Create(strUrl) as HttpWebRequest;
